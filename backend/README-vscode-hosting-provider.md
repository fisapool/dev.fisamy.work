...
3. Boot:
   ...
   docker compose up -d
   ```
4. Test: `https://alice.code.fisamy.work` → enter user/pass → OpenVSCode loads.

**Add users:** duplicate the `alice-openvscode` service in compose, e.g. `bob-openvscode`, then add a new site block in Caddyfile for `bob.code.fisamy.work` pointing to `bob-openvscode:3000` with its own `basicauth` user+hash.
...
What you get:
- Coder UI at `https://code.fisamy.work`
- Wildcard dev‑URLs `*.code.fisamy.work` for workspace apps (VSCode, Jupyter, web servers) routed through Coder
...
export CODER_ACCESS_URL="https://code.fisamy.work"
...
Open `https://code.fisamy.work` → sign in.
...
```
vscode--myworkspace--username.code.fisamy.work
```
...
- Callback URL: `https://code.fisamy.work/oidc/callback`
- Allowed redirect origins: `https://code.fisamy.work`
...
- UptimeKuma checks for `https://code.fisamy.work/healthz` (Coder exposes health endpoints).
...
Yes, for Coder dev‑URLs. Both `code.fisamy.work` and `*.code.fisamy.work` should resolve to your server.
